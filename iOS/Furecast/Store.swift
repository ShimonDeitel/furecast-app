...
/// StoreKit 2 — auto-renewable subscription (`com.shimondeitel.furecast.pro.monthly`).
...
final class Store: ObservableObject {
    static let productID = "com.shimondeitel.furecast.pro.monthly"
    ...
    private var debugForcePro: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["FURECAST_FORCE_PRO"] == "1"
        #else
        return false
        #endif
    }
    ...
    init() {
        ...
        Task {
            #if DEBUG
            if ProcessInfo.processInfo.environment["FURECAST_NO_SK"] != "1" { await loadProduct() }
            #else
            await loadProduct()
            #endif
